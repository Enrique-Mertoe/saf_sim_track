...
export const BulkSIMCardUpload: React.FC<BulkUploadProps> = ({ currentUser }) => {
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle>Bulk SIM Card Registration</CardTitle>
        <CardDescription>
          Upload multiple SIM cards at once using CSV or Excel file.
          <Button variant="link" className="p-0 h-auto text-blue-500" onClick={downloadTemplate}>
            Download template
          </Button>
        </CardDescription>
      </CardHeader>
      <CardContent>
        {!file && (
          ...
          >
            ...
            >
              ...
              <p className="text-xs text-gray-500">CSV or Excel files (up to 5MB)</p>
            </label>
          </div>
        )}
        ...
        {processedResult && (
          <div className="space-y-5">
            <div className={`p-4 ${processedResult.failedCount === 0 ? 'bg-green-50' : 'bg-yellow-50'} rounded-lg border ${processedResult.failedCount === 0 ? 'border-green-200' : 'border-yellow-200'}`}>
              <h3 className={`text-lg font-medium ${processedResult.failedCount === 0 ? 'text-green-800' : 'text-yellow-800'}`}>
              ...
              <div className="mt-3 grid grid-cols-3 gap-4">
                <div className="text-center">
                  ...
                  <p className="text-2xl font-bold">{processedResult.totalRecords}</p>
                </div>
                <div className="text-center">
                  <p className="text-sm text-gray-500">Successful</p>
                  <p className="text-2xl font-bold text-green-600">{processedResult.successCount}</p>
                </div>
                <div className="text-center">
                  <p className="text-sm text-gray-500">Failed</p>
                  <p className="text-2xl font-bold text-red-600">{processedResult.failedCount}</p>
                </div>
              </div>
            </div>
            ...
            {processedResult.errors.length > 0 && (
              <div className="max-h-60 overflow-y-auto p-4 bg-red-50 border border-red-200 rounded-lg">
                ...
                <ul className="space-y-1 list-disc list-inside text-sm text-red-700">
            ...
            <div className="flex justify-end space-x-3">
              {processedResult.successCount > 0 && (
                <Button variant="outline" onClick={() => window.location.href = '/sim-cards'}>
                  View SIM Cards
                </Button>
              )}
