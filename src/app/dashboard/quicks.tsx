...
export const UserStatistics: FC = () => {
  ...
  useEffect(() => {
    const fetchUserStats = async (): Promise<void> => {
      try {
        ...
        // Get team leaders
        const { data: teamLeaders, error: teamLeadersError } = await userService.getUsersByRole(UserRole.TEAM_LEADER);
    ...
    return (
      <div className={cardClasses}>
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center">
            <div className={`p-2 ${iconBg} rounded-full mr-4 transform transition-transform duration-300 hover:scale-110`}>
              {icon}
            </div>
            <div>
  ...
  return (
    <div className={`bg-white p-2 rounded-lg shadow-md transform transition-all duration-700 ${animate ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
