...
export default function TransferSimCard({
    ...
    return (
        <div className="bg-white overflow-y-auto rounded-lg p-6 w-full">
            <h3 className="text-lg font-semibold mb-4">Transfer SIMs to Another Team Member</h3>
            <p className="text-sm text-gray-600 mb-4">
                You're about to transfer {simCards.length} SIM card{simCards.length !== 1 ? 's' : ''} to another team member.
            </p>
            <MaterialSelect
                className={"mb-4"}
                ...
                <button
                    onClick={async () => {
                        ...
                        try {
                            ...
                            alert.success(`Successfully transferred ${simCards.length} SIM card${simCards.length !== 1 ? 's' : ''}`);
                            setSelectedMember('');
                            onClose()
                        } catch (error) {
                            console.error("Error transferring SIMs:", error);
                            alert.error("Failed to transfer SIM cards. Please try again.");
                        } finally {
