...
import {Check, ChevronLeft, ChevronRight, Clock, UserCheck, X} from 'lucide-react';
...
const SimCardGrid = ({
    ...
    itemsPerPage = 12,
    onAssign,
    status = null, // New prop to determine which status to display (null = unassigned)
    onUnassign = null, // New prop for unassigning SIM cards (for assigned tab)
    onTransfer = null // New prop for transferring SIM cards to another user
}) => {
    ...
    // Determine if we're in assigned or unassigned mode
    const isAssignedMode = status === SIMStatus.ASSIGNED;

    // Filter SIM cards based on status
    const filteredSims = useMemo(() => {
        return simCards.filter(sim => {
            // If status is provided, filter by that status
            const statusMatch = status ? sim.status === status : sim.status !== SIMStatus.ASSIGNED;
            ...
            return statusMatch && otherFiltersMatch;
        });
    }, [simCards, selectedStaff, searchTerm, filterSimCards, SIMStatus, status]);
    ...
    // Handle action button click (assign, unassign, or transfer)
    const handleAction = useCallback(() => {
        if (selectedSims.size === 0) return;

        const selectedSimsArray = Array.from(selectedSims);

        if (!isAssignedMode && onAssign) {
            // Handle assign action
            onAssign(selectedSimsArray);
            clearSelection();
        }
    }, [selectedSims, onAssign, isAssignedMode, clearSelection]);
    ...
    const handleUnassign = useCallback(() => {
        ...
        if (isAssignedMode && onUnassign) {
            // Handle unassign action
            onUnassign(selectedSimsArray);
            clearSelection();
        }
    }, [selectedSims, onUnassign, isAssignedMode, clearSelection]);
    ...
    const handleTransfer = useCallback(() => {
        ...
        if (isAssignedMode && onTransfer) {
            // Handle transfer action
            onTransfer(selectedSimsArray);
            clearSelection();
        }
    }, [selectedSims, onTransfer, isAssignedMode, clearSelection]);
    ...
    // Get appropriate status text and colors based on mode
    const getStatusInfo = () => {
        if (isAssignedMode) {
            return {
                icon: UserCheck,
                ...
                statusText: 'Assigned',
                emptyTitle: 'No Assigned SIMs',
                emptyMessage: 'There are no assigned SIM cards that match your current filters.',
                actionText: 'Unassign Selected'
            };
        } else {
            return {
                icon: Clock,
                ...
                emptyTitle: 'No Unassigned SIMs',
                emptyMessage: 'All SIM cards have been assigned or there are no matches for your current filters.',
                actionText: 'Assign Selected'
            };
        }
    };
    ...
    if (filteredSims.length === 0) {
        return (
            <div className="text-center px-6 py-12">
                <StatusIcon className="w-16 h-16 mx-auto mb-4 text-gray-300 dark:text-gray-600"/>
                <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">{statusInfo.emptyTitle}</h3>
                <p className="text-gray-500 dark:text-gray-400">{statusInfo.emptyMessage}</p>
            </div>
        );
    }
        ...
        <div className="space-y-6">
                            ...
                            {isAssignedMode ? (
                                ...
                                onAssign && (
                                    <button
                                        ...
                                        className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-md transition-colors"
                                    >
                                        {statusInfo.actionText} ({selectedSims.size})
                                    </button>
                                )
                            )}
                        </div>
            ...
            <div className="flex px-6 justify-between items-center">
                <p className="text-sm text-gray-600 dark:text-gray-400">
                    Showing {startIndex + 1} to {Math.min(endIndex, filteredSims.length)} of {filteredSims.length} {isAssignedMode ? 'assigned' : 'unassigned'} SIMs
                </p>
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="flex justify-between  items-center">
                                    {isAssignedMode && sim.assigned_on && (
